...
export function buildInfoPrompt(): string {
  return '몇 명이 사용하시나요? 참석자 이름도 함께 알려주세요.\n예: 4명, 홍길동, 김철수';
}
...
export function buildNoRoomMessage(parsedTime: Date, capacity: number): string {
  return `😔 ${formatDateTime(parsedTime)}에 ${capacity}인 이상 수용 가능한 빈 회의실이 없습니다.`;
}

export function buildFocusingRoomList(rooms: Room[]): string {
  const list = rooms.map((r, i) => `${i + 1}) ${r.name}`).join('\n');
  return `사용 가능한 포커싱룸 목록입니다:\n${list}\n\n번호를 선택해주세요.`;
}

export function buildFocusingCapacityWarning(): string {
  return '포커싱룸은 1인용 공간입니다. 인원 설정이 필요 없습니다.';
}
...
export async function processConversationReply(params: ConversationReplyParams): Promise<string> {
  ...
  switch (state.stage) {
    ...
    case 'waiting_focusing_select': {
      const num = parseInt(text.trim(), 10);

      // 포커싱룸 목록 조회
      const availableFocusing = await getAvailableRooms(state.parsedTime, state.endTime, 1);
      const availableFocusingFiltered = availableFocusing.filter(r => r.type === 'focusing');

      if (availableFocusingFiltered.length === 0) {
        clearConversation(userId, channelId);
        return '😔 해당 시간대에 사용 가능한 포커싱룸이 없습니다.';
      }

      if (isNaN(num) || num < 1 || num > availableFocusingFiltered.length) {
        return `1~${availableFocusingFiltered.length} 사이의 번호를 입력해주세요.`;
      }

      const selectedFocusingRoom = availableFocusingFiltered[num - 1];
      updateConversation(userId, channelId, {
        stage: 'waiting_title',
        selectedRoom: selectedFocusingRoom,
        capacity: 1,
        attendees: [],
      });
      return buildTitlePrompt();
    ...
    case 'waiting_edit_date': {
      ...
      return `어느 회의실의 예약을 수정/취소하시겠습니까?\n${roomList}\n\n번호를 입력해주세요.`;
    }
    ...
    case 'waiting_edit_select': {
      ...
      clearConversation(userId, channelId);
      editBookingsCache.delete(getKey(userId, channelId));
      return `예약 수정은 \`/edit\` 커맨드를 사용해주세요. 더 편리하게 수정할 수 있습니다.`;
    }
...
// 포커싱룸 대화 시작 헬퍼
export async function startFocusingConversation(
  userId: string,
  channelId: string,
  parsedTime: Date,
  endTime: Date,
  ...
  const focusingRooms = availableRooms.filter(r => r.type === 'focusing');

  if (focusingRooms.length === 0) {
    return '😔 해당 시간대에 사용 가능한 포커싱룸이 없습니다.';
  }

  startConversation(userId, channelId, 'focusing', parsedTime, endTime, 1, []);
  updateConversation(userId, channelId, { stage: 'waiting_focusing_select' });

  return buildFocusingRoomList(focusingRooms);
}
