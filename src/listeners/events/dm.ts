    ...
    try {
      ...
      if (!intent) {
        ...
        await client.chat.postMessage({
          channel: channelId,
          text: '안녕하세요! 회의실 예약을 도와드립니다.\n예: "내일 오후 2시에 4명 회의실 예약해줘"',
        });
        return;
      }

      // isEditIntent: 수정 의도 → /edit 안내
      ...
      if (intent.isBookingIntent) {
        // 포커싱룸 요청
        if (intent.roomType === 'focusing') {
          const reply = await startFocusingConversation(
            userId,
            channelId,
            intent.parsedTime,
            intent.endTime,
