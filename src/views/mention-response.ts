...
export function buildMentionResponse(
  ...
  const headerText = capacity
    ? `🏢 *${formatDateTime(parsedTime)} 기준 ${capacity}인 이상 사용 가능한 회의실*`
    : `🏢 *${formatDateTime(parsedTime)} 기준 사용 가능한 회의실*`;
...
/**
 * 사용 가능한 회의실이 없을 때의 응답 메시지
 */
export function buildNoRoomResponse(
  parsedTime: Date,
  capacity: number | null,
): object[] {
  const capacityText = capacity ? `(${capacity}인 이상 수용 가능한 회의실 기준)` : '';
  const text = `😔 ${formatDateTime(parsedTime)} 기준으로 조건에 맞는 빈 회의실이 없습니다.${capacity ? ` ${capacityText}` : ''}`;
